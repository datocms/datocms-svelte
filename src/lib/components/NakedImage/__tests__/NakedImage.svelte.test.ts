...
describe('NakedImage', () => {
		...
		it('renders correctly', () => {
			...
			const { container, component } = render(NakedImage, {
				props: {
					data: minimalData,
					class: 'class-name',
					style: 'background: red; overflow: visible; padding: 0px 10px;'
				}
			});
	...
	describe('full data', () => {
		it('renders correctly', () => {
			const { container } = render(NakedImage, { props: { data: completeData } });
			expect(container).toMatchSnapshot();
		});
	});
	...
	describe('minimal data', () => {
		it('renders correctly', () => {
			const { container } = render(NakedImage, { props: { data: minimalData } });
			expect(container).toMatchSnapshot();
		});
	});
